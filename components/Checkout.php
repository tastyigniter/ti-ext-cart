...
class Checkout extends BaseComponent
{
    ...
    protected function createRules()
    {
        ...
        if (Location::orderTypeIsDelivery()) {
            ...
            $namedRules[] = ['address.address_2', 'lang:igniter.cart::default.checkout.label_address_2', ($this->property('showAddress2Field', 1) ? 'sometimes|' : '').'min:3|max:128'];
            $namedRules[] = ['address.city', 'lang:igniter.cart::default.checkout.label_city', ($this->property('showCityField', 1) ? 'sometimes|' : '').'min:2|max:128'];
            $namedRules[] = ['address.state', 'lang:igniter.cart::default.checkout.label_state', ($this->property('showStateField', 1) ? 'sometimes|' : '').'max:128'];
            $namedRules[] = ['address.postcode', 'lang:igniter.cart::default.checkout.label_postcode', 'string'];
            $namedRules[] = ['address.country_id', 'lang:igniter.cart::default.checkout.label_country', ($this->property('showCountryField', 1) ? 'sometimes|' : '').'required|integer'];
        }
